...
/* ── Stat card with animated counter & hover glow ── */
...
const QuickBtn = ({ to, icon, label, gradient }) => (
  <Link to={to} style={{ textDecoration: 'none', flex: '1 1 auto' }}>
    <motion.div 
      whileHover={{ y: -4, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', borderColor: 'rgba(225, 29, 72, 0.3)' }}
      whileTap={{ scale: 0.98 }}
      style={{
  ...
  return (
    <Layout title="Student Dashboard">
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, position: 'relative', zIndex: 1 }}>
            ...
            <div className="cg-banner-actions">
              <Link to="/student/submit" className="btn-cg-primary btn-glow">
        ...
        <div className="row g-3 mb-4">
          {[
            { icon: '📋', value: stats.total || 0,    label: 'Total Complaints', gradient: 'linear-gradient(135deg,#e11d48,#dc2626)', delay: 0 },
            { icon: '⏳', value: stats.pending || 0,   label: 'Pending',          gradient: 'linear-gradient(135deg,#f59e0b,#fb923c)', delay: 0.08 },
            { icon: '🔄', value: stats.inProgress || 0,label: 'In Progress',      gradient: 'linear-gradient(135deg,#dc2626,#e11d48)', delay: 0.16 },
            { icon: '✅', value: stats.resolved || 0,  label: 'Resolved',         gradient: 'linear-gradient(135deg,#16a34a,#15803d)', delay: 0.24 },
          ].map((s) => (
        ...
        >
          ...
          <div className="cg-quick-actions" style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
            <QuickBtn to="/student/submit"     icon="✏️" label="Submit Complaint" gradient="linear-gradient(135deg,#e11d48,#be123c)" />
            <QuickBtn to="/student/complaints" icon="📋" label="My Complaints"   gradient="linear-gradient(135deg,#16a34a,#115e59)" />
            <QuickBtn to="/track"              icon="🔍" label="Track Anonymous"  gradient="linear-gradient(135deg,#dc2626,#16a34a)" />
          </div>
        </motion.div>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
            ...
            <Link to="/student/complaints" style={{ fontSize: 13, color: '#fb7185', textDecoration: 'none' }}>
          ...
          <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
              ...
              <Link to="/student/submit" style={{ color: '#fb7185', fontSize: 13 }}>Submit your first complaint →</Link>
            </div>
          ) : (
              ...
              <table className="cg-table">
                ...
                <tbody>
                  {data.recentComplaints.map((c) => (
                    ...
                    >
                      <td data-label="Ref"><span style={{ fontFamily: 'monospace', fontSize: 12, color: '#fb7185' }}>{c.referenceNumber}</span></td>
                      ...
                      <td data-label="">
                        <Link to={`/student/complaints/${c._id}`} className="btn btn-outline-primary btn-sm btn-glow" style={{ borderRadius: 8, fontSize: 12 }}>
                          View
                        </Link>
                      </td>
                    </motion.tr>
