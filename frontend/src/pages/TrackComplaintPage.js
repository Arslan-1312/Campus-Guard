  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', padding: '40px 16px' }}>
      ...
      <div className="text-center mb-4 mt-3">
        ...
        <h2 style={{ fontWeight: 800, color: 'var(--text-primary)' }}>Track Your Complaint</h2>
        <p style={{ color: 'var(--text-secondary)' }}>Enter your reference number to check the status</p>
      </div>
        ...
        {complaint && (
          <div className="cg-card mt-3">
            <div className="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
              <div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>Reference Number</div>
                <div style={{ fontFamily: 'monospace', fontWeight: 700, color: 'var(--violet-light)', fontSize: 15 }}>{complaint.referenceNumber}</div>
              </div>
              <StatusBadge status={complaint.status} />
            </div>

            <h5 style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{complaint.title}</h5>
            ...
            {complaint.statusHistory?.length > 0 && (
              <div className="mt-4">
                <h6 style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>Status History</h6>
            ...
            {complaint.comments?.length > 0 && (
              <div className="mt-4">
                <h6 style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>Updates from Administration</h6>
                {complaint.comments.map((c, i) => (
                  <div key={i} style={{ background: 'var(--bg-base)', border: '1px solid var(--glass-border)', borderRadius: 8, padding: 12, marginBottom: 8 }}>
                    <div style={{ fontSize: 12, color: 'var(--violet-light)', fontWeight: 600, marginBottom: 4 }}>
                      <i className="bi bi-shield-check me-1" />
                      {c.authorRole?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                    </div>
                    <div style={{ fontSize: 14, color: 'var(--text-primary)' }}>{c.text}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{new Date(c.createdAt).toLocaleString()}</div>
                  </div>
                ))}
              </div>
            )}
